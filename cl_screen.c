...
cvar_t scr_loadingscreen_maxfps = {0, "scr_loadingscreen_maxfps", "30", "restrict maximal FPS for loading screen so it will not update very often (this will make lesser loading times on a maps loading large number of models)"};
cvar_t scr_loadingscreen_indicator = {0, "scr_loadingscreen_indicator", "", "spinning indicator to display in the bottom right corner during loading"};
cvar_t scr_infobar_height = {0, "scr_infobar_height", "8", "the height of the infobar items"};
cvar_t vid_conwidth = {CVAR_SAVE, "vid_conwidth", "960", "virtual width of 2D graphics system"};
cvar_t vid_conheight = {CVAR_SAVE, "vid_conheight", "540", "virtual height of 2D graphics system"};
cvar_t vid_pixelheight = {0, "vid_pixelheight", "1", "adjusts vertical field of vision to account for non-square pixels (1280x1024 on a CRT monitor for example)"};
cvar_t scr_aspectname = {0, "scr_aspectname", "16-9", "string name for the current aspect ratio; use a dash instead of a colon, e. g. 16-9"};
...
void CL_Screen_Init(void)
{
	...
	Cvar_RegisterVariable (&scr_loadingscreen_indicator);
	...
	Cvar_RegisterVariable (&scr_aspectname);
#ifdef CONFIG_VIDEO_CAPTURE
...
//	CHECKGLERROR
	...
	if (gamemode == GAME_WRATH)
	{
		...
		if (loadingscreenind)
		{
			...
			loadingscreenind_vertex3f[0] = loadingscreenind_vertex3f[9] = -loadingscreenind->width / 2;
			loadingscreenind_vertex3f[1] = loadingscreenind_vertex3f[4] = -loadingscreenind->height / 2;
			loadingscreenind_vertex3f[3] = loadingscreenind_vertex3f[6] = loadingscreenind->width / 2;
			loadingscreenind_vertex3f[7] = loadingscreenind_vertex3f[10] = loadingscreenind->height / 2;
...
void SCR_UpdateLoadingScreen (qboolean clear, qboolean startup)
{
	...
	loadingscreenind_angle += 5.f;
	loadingscreenind_show = (int)t & 0x1;
...
void CL_UpdateScreen(void)
{
	...
	{
		SCR_UpdateLoadingScreen(true, false);
		return;
	}
