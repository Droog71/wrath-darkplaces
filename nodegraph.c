...
#define nodegraph_links_sub_matrix_LENGTH (NODEGRAPH_NODES_COUNT_LIMIT * NODEGRAPH_NODES_COUNT_LIMIT / 8)
...
typedef struct nodegraph_s
{
    ...
    char links[nodegraph_links_sub_matrix_LENGTH];
    int nodes_count;
}
nodegraph_t;
...
qboolean nodegraph_moveprobe_fly(const vec3_t nodefrom, const vec3_t nodeto, const vec3_t mins, const vec3_t maxs, short type)
{
    ...
    trace = SV_TraceBox(from, mins, maxs, to, MOVE_NOMONSTERS, NULL, contents, 0.0f);
